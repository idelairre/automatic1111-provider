...
import { ComfyUIModelNotFoundError, ComfyUIAPIError, ComfyUITimeoutError, ComfyUIAbortError, ComfyUINetworkError, } from './errors.js';
/**
 * Resolve a model ID to its checkpoint filename for ComfyUI
 * Follows naming convention: lowercase, dashes, no extensions
 */
export function resolveModelCheckpoint(modelId) {
 ...
 */
export function createModelSchema(checkpoint) {
    return {
        id: checkpoint,
 ...
 */
export async function checkpointExists(baseURL, checkpointName) {
    try {
 ...
 */
export function createComfyUIImageModel(modelId, settings = {}, providerSettings = {}) {
    const config = {
        provider: 'comfyui',
...
class ComfyUIImageModel {
    ...
    async doGenerate({ prompt, n, size, aspectRatio, seed, providerOptions, headers, abortSignal, }) {
        ...
        const { negativePrompt, styles, steps, cfgScale, sampler, denoisingStrength, checkModelExists, ...providerRequestOptions } = providerOptions?.comfyui || {};
        const settings = {
    ...
    async generateImages({ prompt, count = 1, size: _size, seed, settings, headers, abortSignal, }) {
        ...
        const isSDXL = checkpointName.toLowerCase().includes('xl') || checkpointName.toLowerCase().includes('sdxl');
            ...
            try {
                ...
                if (historyResponse.ok) {
                    ...
                    if (history[promptId]?.outputs?.['7']?.images) {
                        imageUrls = history[promptId].outputs['7'].images.map((img) => `${baseURL}/view?filename=${img.filename}&subfolder=${img.subfolder}&type=${img.type}`);
                        break;
                    }
                }
                else {
        ...
        const imagePromises = imageUrls.map(async (url) => {
            ...
            if (!response.ok) {
                throw new ComfyUINetworkError(`Failed to download image: ${response.status} ${response.statusText}`);
            }
...
export function createComfyUIProvider(options = {}) {
    ...
    const createImageModel = (modelId, settings) => createComfyUIImageModel(modelId, settings, providerSettings);
    return {
